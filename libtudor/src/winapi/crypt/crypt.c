...
__winfnc BOOL CryptAcquireContextW(struct crypt_provider **prov, const char16_t *cont_name, const char16_t *prov_name, DWORD prov_type, DWORD flags) {
    char *cont_name_a = cont_name ? winstr_to_str(cont_name) : NULL;
    char *prov_name_a = prov_name ? winstr_to_str(prov_name) : NULL;
    BOOL result = CryptAcquireContextA(prov, cont_name_a, prov_name_a, prov_type, flags);
    free(cont_name_a);
    free(prov_name_a);
    return result;
}
